...
export const getDashboard = async (
  ...
  try {
    ...
    const _domainProgress = await prisma.userProgress.findMany({
      where: { userId },
      include: {
        domain: {
          select: {
    ...
    const weeklyData = Object.values(dailyPerformance).map((day) => ({
      ...day,
      accuracy:
        day.total > 0 ? Math.round((day.correct / day.total) * 100) : null,
    }));
...
export const getDomainProgress = async (
  ...
  try {
    ...
    for (const answer of incorrectAnswers) {
      questionMissCount[answer.questionId] =
        (questionMissCount[answer.questionId] || 0) + 1;
    }
