...
export async function checkAccountLockout(email: string): Promise<LockoutStatus> {
    ...
    try {
        ...
        const lockoutData = await cache.get<LockoutData>(KEYS.locked(normalizedEmail));
        ...
        const attemptsData = await cache.get<number>(KEYS.attempts(normalizedEmail));
...
export async function recordFailedAttempt(email: string): Promise<LockoutStatus> {
    ...
    try {
        ...
        Logger.warn(`Failed login attempt ${attempts}/${LOCKOUT_CONFIG.maxAttempts} for: ${normalizedEmail}`);
...
async function lockAccount(email: string): Promise<LockoutStatus> {
    try {
        ...
        await cache.set(KEYS.lockoutCount(email), lockoutCount + 1, 7 * 24 * 60 * 60);

        // Clear attempt counter
        await cache.del(KEYS.attempts(email));

        Logger.warn(`Account locked: ${email} for ${duration} seconds (lockout #${lockoutCount + 1})`);
