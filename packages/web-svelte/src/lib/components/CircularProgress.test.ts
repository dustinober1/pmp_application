...
describe("CircularProgress Component", () => {
	...
	describe("Basic Rendering", () => {
		...
		it("does not render description when not provided", () => {
			...
			const hasDescription = paragraphs.some((p) => p.tagName === "P" && p.textContent?.length);
	...
	describe("Color Variants", () => {
		...
		it("applies purple color variant", () => {
			...
			const progressCircle = container.querySelector('circle[stroke="#8b5cf6"]');
			expect(progressCircle).toBeInTheDocument();
		});
	...
	describe("Component Structure", () => {
		...
		it("background circle uses proper stroke color", () => {
			...
			expect(backgroundCircle).toHaveClass("text-gray-200", "dark:text-gray-700");
		});
	});
});
