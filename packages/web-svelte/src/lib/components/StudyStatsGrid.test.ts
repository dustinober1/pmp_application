...
describe("StudyStatsGrid Component", () => {
  ...
  describe("localStorage data loading - Mock Exam Average", () => {
    ...
    it("should only consider last 3 exams when more exist", () => {
      ...
      const exams = [
        {
          completedAt: new Date(today.getTime() - 4000).toISOString(),
          scorePercentage: 60,
        },
        {
          completedAt: new Date(today.getTime() - 3000).toISOString(),
          scorePercentage: 70,
        },
        {
          completedAt: new Date(today.getTime() - 2000).toISOString(),
          scorePercentage: 80,
        },
        {
  ...
  describe("streak calculation logic - edge cases", () => {
    ...
    it("should update localStorage to 0 when streak is reset", () => {
      ...
      expect(
        global.window.localStorage.getItem(STORAGE_KEYS.STUDY_STREAK),
      ).toBe("0");
    });
