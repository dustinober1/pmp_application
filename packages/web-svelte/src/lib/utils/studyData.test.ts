...
const mockFlashcardsData = [
	{
		...
		flashcards: [
			{ id: 1, category: 'Leadership', front: 'What is conflict?', back: 'A disagreement' }
		]
	},
	{
		meta: {
		...
		flashcards: [
			{ id: 1, category: 'Planning', front: 'What is scope?', back: 'Project boundaries' }
		]
	},
	{
		meta: {
...
describe('studyData', () => {
	...
	describe('getStudyGuide', () => {
		...
		it('should include overview section', async () => {
			...
			const overviewSection = guide?.sections.find((s) => s.title === 'Overview');
	...
	describe('getStudyStats', () => {
		...
		it('should include domain breakdown', async () => {
			...
			const peopleStats = stats.domainBreakdown.find((d) => d.domainId === 'people');
