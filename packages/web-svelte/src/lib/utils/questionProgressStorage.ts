...
function saveAllQuestionProgress(progress: Record<string, CardProgress>): void {
    ...
    try {
        localStorage.setItem(STORAGE_KEYS.QUESTIONS_CARD_PROGRESS, JSON.stringify(progress));
    } catch (error) {
 ...
 */
export function saveQuestionProgress(questionId: string, progress: CardProgress): void {
    if (typeof window === 'undefined') return;
 ...
 */
export function getOrInitializeQuestionProgress(questionId: string): CardProgress {
...
export function clearAllQuestionProgress(): void {
    ...
    try {
        ...
        console.error('Failed to clear question progress from localStorage:', error);
    }
}
