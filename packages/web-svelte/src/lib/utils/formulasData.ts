...
function safeDivision(
  ...
  if (denominator === 0) {
    throw new FormulaValidationError(
      fieldName,
      `Division by zero: ${fieldName} cannot be zero`,
    );
  }
  ...
  switch (formula.id) {
    ...
    case "pv_fv":
      ...
      if (denominator === 0) {
        throw new FormulaValidationError(
          "denominator",
          "Division by zero: (1 + r)^n cannot be zero",
        );
      }
    ...
    case "npv": {
      ...
      if (!Array.isArray(cashFlows) || cashFlows.length === 0) {
        throw new FormulaValidationError(
          "cashFlows",
          "cashFlows must be a non-empty array",
        );
      }
      ...
      cashFlows.forEach((cf, index) => {
        ...
        if (periodDenominator === 0) {
          throw new FormulaValidationError(
            `period ${period}`,
            "Division by zero: (1 + r)^period cannot be zero",
          );
        }
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        for (let j = 0; j < cashFlows.length; j++) {
          ...
          if (periodDenominator === 0) {
            throw new FormulaValidationError(
              `iteration ${i}`,
              "Division by zero in IRR calculation",
            );
          }
          npv += safeDivision(
            cashFlows[j],
          ...
          if (j > 0) {
            ...
            if (derivativeDenominator === 0) {
              throw new FormulaValidationError(
                `iteration ${i}`,
                "Division by zero in IRR derivative",
              );
            }
            dnpv -= safeDivision(
              j * cashFlows[j],
        ...
        if (dnpv === 0) {
          throw new FormulaValidationError(
            "derivative",
            "Division by zero: derivative cannot be zero in IRR calculation",
          );
        }
    ...
    case "point_of_total_assumption":
      ...
      const ptaQuotient = safeDivision(
        ptaNumerator,
        inputs.buyerShareRatio,
        "buyerShareRatio",
      );
...
export function parseCashFlows(input: string): number[] {
  ...
  const parsed = input.split(",").map((s) => {
    const trimmed = s.trim();
    if (trimmed === "") {
      throw new FormulaValidationError(
        "input",
        "Empty value found in cash flows input",
      );
    }
    ...
    if (!Number.isFinite(num)) {
      throw new FormulaValidationError(
        trimmed,
        `"${trimmed}" is not a finite number`,
      );
    }
    return num;
  });
  ...
  if (parsed.length === 0) {
    throw new FormulaValidationError(
      "input",
      "No valid cash flows found in input",
    );
  }
