...
vi.mock('./cardProgressStorage', () => ({
	...
	saveCardProgress: vi.fn((cardId: string, progress: unknown) => {
		const stored = localStorageMock.getItem('pmp_flashcards_card_progress');
		const allProgress: Record<string, unknown> = stored ? JSON.parse(stored) as Record<string, unknown> : {};
		allProgress[cardId] = progress;
		localStorageMock.setItem('pmp_flashcards_card_progress', JSON.stringify(allProgress));
	}),
	...
	getDueCards: vi.fn((cardIds: string[]) => {
		const stored = localStorageMock.getItem('pmp_flashcards_card_progress');
		const allProgress: Record<string, { nextReviewDate: string }> = stored ? JSON.parse(stored) as Record<string, { nextReviewDate: string }> : {};
		const now = new Date();
...
describe('spacedRepetition', () => {
	...
	describe('calculateSM2', () => {
		...
		describe('Rating: "easy" (quality 5)', () => {
			...
			it('should increase ease factor more than "good"', () => {
				...
				expect(resultEasy.newEaseFactor).toBeGreaterThan(resultGood.newEaseFactor);
	...
	describe('Ease Factor Calculations', () => {
		...
		it('should handle ease factor at boundary (1.3)', () => {
			...
			expect(result.newEaseFactor).toBeGreaterThanOrEqual(SM2_DEFAULTS.MINIMUM_EASE_FACTOR);
		});
		...
		it('should calculate ease factor correctly for all quality levels', () => {
			...
			testCases.forEach(({ rating, expectedChange }) => {
				...
				const expectedEF = Math.max(SM2_DEFAULTS.MINIMUM_EASE_FACTOR, initialEF + expectedChange);
	...
	describe('updateCardProgress', () => {
		...
		it('should calculate next review date correctly', () => {
			...
			expect(nextReview.getTime()).toBeGreaterThanOrEqual(expectedMin.getTime());
		...
		it('should persist progress to localStorage', () => {
			...
			const allProgress = JSON.parse(stored as string) as Record<string, unknown>;
	...
	describe('Edge Cases', () => {
		...
		it('should handle transition from failed to successful', () => {
			...
			// Succeed
			const result2 = calculateSM2(result1.newEaseFactor, result1.newInterval, result1.newRepetitions, 'good');
