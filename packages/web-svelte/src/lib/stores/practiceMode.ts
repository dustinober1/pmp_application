...
import {
  ...
  isQuestionDue,
} from "$lib/utils/questionProgressStorage";
...
function createPracticeModeStore() {
  ...
  return {
    ...
    startSession(
      ...
      options?: {
        limit?: number;
        priority?: "srs" | "shuffle" | "flagged" | "none";
      },
    ) {
      ...
      if (options?.priority === "srs") {
        selectedQuestions.sort((a, b) => {
          ...
          if (!progB) return 1;
          return (
            new Date(progA.nextReviewDate).getTime() -
            new Date(progB.nextReviewDate).getTime()
          );
        });
      } else if (options?.priority === "flagged") {
        // Filter only flagged questions
        selectedQuestions = selectedQuestions.filter(
          (q) => progressRecord[q.id]?.flagged,
        );
      }
...
export const currentQuestion = derived(
  practiceMode,
  ($store) => $store.questions[$store.currentIndex] || null,
);
...
export const sessionStats = derived(practiceMode, ($store) => ({
  ...
  percentage:
    $store.questions.length > 0
      ? Math.round(($store.correctCount / ($store.currentIndex + 1)) * 100)
      : 0,
}));
