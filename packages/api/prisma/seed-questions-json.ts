...
async function main() {
  ...
  const domainMap = new Map(
    domains.map((d) => [d.name.toLowerCase().split(" ")[0], d]),
  );
  ...
  const getDomainId = (jsonDomain: string) => {
    ...
    if (lower === "people")
      return domains.find((d) => d.name.toLowerCase().includes("people"))?.id;
    if (lower === "process")
      return domains.find((d) => d.name.toLowerCase().includes("process"))?.id;
    if (lower === "business")
      return domains.find((d) => d.name.toLowerCase().includes("business"))?.id;
    return null;
  };
  ...
  for (const q of questions) {
    ...
    if (!taskId) {
      console.warn(
        `⚠️  Task not found for question ${q.id} (Task ${q.taskNumber})`,
      );
      totalSkipped++;
      continue;
    }
