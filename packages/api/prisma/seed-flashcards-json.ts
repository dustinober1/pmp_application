...
async function main() {
    ...
    const normalizeValues = (str: string) => str.split('(')[0].trim().toLowerCase();
    const smartDomainMap = new Map(domains.map(d => [normalizeValues(d.name), d.id]));
    ...
    for (const set of flashcardSets) {
        ...
        if (!domainId) {
            ...
            else if (lowerName.includes('process')) domainId = smartDomainMap.get('process');
            else if (lowerName.includes('business')) domainId = smartDomainMap.get('business environment');
        }
        ...
        let task = tasks.find(t => t.name.toLowerCase() === taskNameRaw.toLowerCase());
        ...
        if (!task && set.meta.ecoReference) {
            ...
            if (codeLookup) {
                const foundTask = await prisma.task.findUnique({ where: { code: codeLookup } });
                if (foundTask) task = foundTask;
            }
        }

        if (!task) {
            console.warn(`⚠️  Task not found: "${taskNameRaw}" in domain "${domainNameRaw}" - Skipping set.`);
            continue;
        }

        console.log(`Processing set for: ${domainNameRaw} -> ${task.name} (${set.flashcards.length} cards)`);
