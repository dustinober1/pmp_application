...
async function main() {
  console.log(
    "🌱 Seeding database with 2026 PMP Examination Content Outline...",
  );
  ...
  try {
    ...
    const introCsvPath = path.join(
      __dirname,
      "../../../flashcards/01_introduction.csv",
    );
    if (fs.existsSync(introCsvPath)) {
      ...
      for (const line of lines) {
        ...
        else if (domainCode === "BUSINESS_ENVIRONMENT")
          taskId = taskMap.get("III.1");
    ...
    if (fs.existsSync(businessTask1CsvPath)) {
      ...
      if (businessDomainId && businessTask1Id) {
        ...
        console.log(
          `    ✅ Imported ${count} Business Task 1 (Governance) flashcards`,
        );
      }
    } else {
