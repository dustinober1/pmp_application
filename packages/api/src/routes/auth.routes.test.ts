...
describe("Auth Routes Integration Tests", () => {
  ...
  describe("GET /api/auth/csrf", () => {
    it("should issue a CSRF token and set cookie", async () => {
      ...
      expect(csrfCookie).toContain(
        `pmp_csrf_token=${response.body.data.csrfToken}`,
      );
    });
  });
  ...
  describe("POST /api/auth/logout", () => {
    ...
    it("should return 401 when user does not exist", async () => {
      const token = jwt.sign(
        { userId: "non-existent", email: "user@example.com" },
        env.JWT_SECRET,
      );
  ...
  describe("GET /api/auth/me", () => {
    ...
    it("should return 401 when user does not exist", async () => {
      const token = jwt.sign(
        { userId: "deleted-user", email: "user@example.com" },
        env.JWT_SECRET,
      );
    ...
    it("should return 403 when user account is locked", async () => {
      ...
      expect(response.status).toBe(403);
      expect(response.body.error.code).toBe(AUTH_ERRORS.AUTH_004.code);
      expect(response.body.error.suggestion).toContain(
