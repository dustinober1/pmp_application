...
const mockSearchResults = [
  {
    ...
    highlightedExcerpt:
      "...The PMP exam has been updated to include **agile**...",
    relevanceScore: 15,
  },
  {
    chapterSlug: "01-introduction",
  ...
  },
  {
    ...
    highlightedExcerpt:
      "...The project charter is essential for **agile** and predictive...",
    relevanceScore: 10,
  },
];
...
beforeEach(() => {
  ...
  (optionalAuthMiddleware as jest.Mock).mockImplementation(
...
describe("GET /api/ebook", () => {
  ...
  it("should handle service errors", async () => {
    (ebookService.getAllChapters as jest.Mock).mockRejectedValueOnce(
...
describe("GET /api/ebook/chapters/:slug", () => {
  it("should get chapter by slug", async () => {
    (ebookService.getChapterBySlug as jest.Mock).mockResolvedValueOnce(
      mockChapterDetail,
    );

    const response = await request(app).get(
      "/api/ebook/chapters/01-introduction",
    );
    ...
    expect(ebookService.getChapterBySlug).toHaveBeenCalledWith(
      "01-introduction",
    );
  });
...
describe("GET /api/ebook/chapters/:chapterSlug/sections/:sectionSlug", () => {
  it("should get section content with access", async () => {
    (ebookService.getSectionBySlug as jest.Mock).mockResolvedValueOnce(
      mockSection,
    );
  ...
  it("should update progress for authenticated user", async () => {
    ...
    (ebookProgressService.updateProgress as jest.Mock).mockResolvedValueOnce(
      mockProgress,
    );
  ...
  it("should return 404 when chapterSlug is empty (route not matched)", async () => {
    const response = await request(app).get(
      "/api/ebook/chapters//sections/understanding-exam",
    );
  ...
  it("should return 404 when sectionSlug is empty (route not matched)", async () => {
    const response = await request(app).get(
      "/api/ebook/chapters/01-introduction/sections/",
    );
...
describe("GET /api/ebook/search", () => {
  ...
  it("should search with premium user tier", async () => {
    ...
    expect(response.status).toBe(200);
    expect(ebookService.searchContent).toHaveBeenCalledWith(
      "stakeholder",
      "pro",
      {
        page: 1,
  ...
  it("should handle service errors during search", async () => {
    (ebookService.searchContent as jest.Mock).mockRejectedValueOnce(
  ...
  it("should return results sorted by relevance", async () => {
    const resultsByRelevance = {
      results: [...mockSearchResults].sort(
...
describe("GET /api/ebook/progress", () => {
  it("should get user progress", async () => {
    (ebookProgressService.getProgress as jest.Mock).mockResolvedValueOnce(
      mockProgress,
    );
...
describe("GET /api/ebook/progress/chapter/:chapterSlug", () => {
  ...
  it("should get chapter progress", async () => {
    (
      ebookProgressService.getChapterProgress as jest.Mock
    ).mockResolvedValueOnce(mockChapterProgress);

    const response = await request(app).get(
      "/api/ebook/progress/chapter/01-introduction",
    );
  ...
  it("should handle service errors", async () => {
    (
      ebookProgressService.getChapterProgress as jest.Mock
    ).mockRejectedValueOnce(new Error("Chapter not found"));
...
describe("POST /api/ebook/progress/complete", () => {
  it("should mark section as complete", async () => {
    (
      ebookProgressService.markSectionComplete as jest.Mock
...
describe("POST /api/ebook/progress/reset", () => {
  it("should reset all progress", async () => {
    (ebookProgressService.resetProgress as jest.Mock).mockResolvedValueOnce(
      undefined,
    );
...
describe("Freemium Access Control - Section Content", () => {
  ...
  describe("Free User (no subscription)", () => {
    beforeEach(() => {
      ...
      (ebookProgressService.updateProgress as jest.Mock).mockResolvedValue(
        undefined,
      );
    });

    test.each(freeChapterSlugs)(
      "should allow access to free chapter: %s",
      async (chapterSlug) => {
