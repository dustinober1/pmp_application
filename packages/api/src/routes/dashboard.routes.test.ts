...
beforeEach(() => {
  ...
  (requireTier as jest.Mock).mockReturnValue(
...
describe("GET /api/dashboard", () => {
  it("should get dashboard data", async () => {
    (dashboardService.getDashboardData as jest.Mock).mockResolvedValueOnce(
      mockDashboardData,
    );
  ...
  it("should handle service errors", async () => {
    (dashboardService.getDashboardData as jest.Mock).mockRejectedValueOnce(
      new Error("DB Error"),
    );
...
describe("GET /api/dashboard/progress", () => {
  it("should get domain progress", async () => {
    ...
    (dashboardService.getDomainProgress as jest.Mock).mockResolvedValueOnce(
      mockProgress,
    );
...
describe("GET /api/dashboard/reviews", () => {
  it("should get upcoming reviews with default limit", async () => {
    ...
    expect(dashboardService.getUpcomingReviews).toHaveBeenCalledWith(
      mockUserId,
      10,
    );
  });
...
describe("GET /api/dashboard/weak-areas", () => {
  it("should get weak areas", async () => {
    (dashboardService.getWeakAreas as jest.Mock).mockResolvedValueOnce(
      mockWeakAreas,
    );
...
describe("GET /api/dashboard/readiness", () => {
  it("should get readiness score", async () => {
    (dashboardService.getReadinessScore as jest.Mock).mockResolvedValueOnce(
      mockReadiness,
    );
...
describe("GET /api/dashboard/recommendations", () => {
  it("should get recommendations", async () => {
    (dashboardService.getRecommendations as jest.Mock).mockResolvedValueOnce(
      mockRecommendations,
    );
    ...
    expect(dashboardService.getRecommendations).toHaveBeenCalledWith(
      mockUserId,
    );
  });
});
