...
describe("Team Routes", () => {
  ...
  describe("POST /api/teams", () => {
    it("should create a team with valid data", async () => {
      ...
      const response = await request(app)
        .post("/api/teams")
        .send({ name: "New Team", licenseCount: 20 });

      expect(response.status).toBe(201);
      expect(response.body.data.team).toEqual(mockTeam);
      expect(teamService.createTeam).toHaveBeenCalledWith(mockUserId, {
        name: "New Team",
        licenseCount: 20,
      });
    });
  ...
  describe("GET /api/teams/:id/dashboard", () => {
    it("should get team dashboard", async () => {
      ...
      const response = await request(app).get(
        `/api/teams/${mockTeamId}/dashboard`,
      );
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("POST /api/teams/:id/invitations", () => {
    it("should invite member", async () => {
      (teamService.inviteMember as jest.Mock).mockResolvedValue({
        token: "abc",
      });
      const response = await request(app)
        .post(`/api/teams/${mockTeamId}/invitations`)
  ...
  describe("DELETE /api/teams/:id/members/:memberId", () => {
    it("should remove member", async () => {
      ...
      const response = await request(app).delete(
        `/api/teams/${mockTeamId}/members/${mockMemberId}`,
      );
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("PATCH /api/teams/:id/members/:memberId/role", () => {
    it("should update role", async () => {
      ...
      const response = await request(app)
        .patch(`/api/teams/${mockTeamId}/members/${mockMemberId}/role`)
        .send({ role: "admin" });
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("POST /api/teams/:id/alerts/:alertId/acknowledge", () => {
    it("should acknowledge alert", async () => {
      ...
      const response = await request(app).post(
        `/api/teams/${mockTeamId}/alerts/${mockAlertId}/acknowledge`,
      );
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("Goals", () => {
    ...
    it("should create goal", async () => {
      ...
      const response = await request(app)
        .post(`/api/teams/${mockTeamId}/goals`)
        .send(goalData);
      expect(response.status).toBe(201);
    });
    ...
    it("should delete goal", async () => {
      ...
      const response = await request(app).delete(
        `/api/teams/${mockTeamId}/goals/${mockGoalId}`,
      );
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("DELETE /api/teams/:id/members/:memberId/preserve", () => {
    it("should remove member with preservation", async () => {
      (
        teamService.removeMemberWithDataPreservation as jest.Mock
      ).mockResolvedValue({
        message: "done",
      });
      const response = await request(app).delete(
        `/api/teams/${mockTeamId}/members/${mockMemberId}/preserve`,
      );
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("Reports", () => {
    it("should generate report in json", async () => {
      ...
      const response = await request(app)
        .post(`/api/teams/${mockTeamId}/reports`)
        .send({ format: "json" });
      expect(response.status).toBe(200);
    });
