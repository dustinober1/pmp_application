...
describe("Practice Routes", () => {
  ...
  describe("GET /api/practice/sessions/:id", () => {
    it("should get session by id", async () => {
      (practiceService.getSession as jest.Mock).mockResolvedValue({
        id: mockSessionId,
      });
      const response = await request(app).get(
        `/api/practice/sessions/${mockSessionId}`,
      );
  ...
  describe("POST /api/practice/sessions/:id/answers/:questionId", () => {
    it("should submit answer", async () => {
      (practiceService.submitAnswer as jest.Mock).mockResolvedValue({
        correct: true,
      });
      const response = await request(app)
        .post(
          `/api/practice/sessions/${mockSessionId}/answers/${mockQuestionId}`,
        )
        .send({
          selectedOptionId: "123e4567-e89b-12d3-a456-426614174999",
          timeSpentMs: 1000,
        });
  ...
  describe("POST /api/practice/sessions/:id/complete", () => {
    it("should complete session", async () => {
      ...
      const response = await request(app).post(
        `/api/practice/sessions/${mockSessionId}/complete`,
      );
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("Flagged Questions", () => {
    ...
    it("should flag a question", async () => {
      ...
      const response = await request(app).post(
        `/api/practice/questions/${mockQuestionId}/flag`,
      );
      expect(response.status).toBe(200);
    });

    it("should unflag a question", async () => {
      (practiceService.unflagQuestion as jest.Mock).mockResolvedValue(
        undefined,
      );
