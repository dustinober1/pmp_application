...
beforeEach(() => {
  ...
  (requireFeature as jest.Mock).mockReturnValue((_req: any, _res: any, next: any) => next());
});

describe('GET /api/flashcards', () => {
  it('should get flashcards without filters', async () => {
    const mockFlashcards = [mockFlashcard, mockCustomFlashcard];
    (flashcardService.getFlashcards as jest.Mock).mockResolvedValueOnce(mockFlashcards);
  ...
  it('should get flashcards with domainId filter', async () => {
    ...
    const response = await request(app).get(`/api/flashcards?domainId=${mockDomainId}`);
  ...
  it('should get flashcards with taskId filter', async () => {
    ...
    const response = await request(app).get(`/api/flashcards?taskId=${mockTaskId}`);
  ...
  it('should return 400 for invalid domainId UUID', async () => {
    const response = await request(app).get('/api/flashcards?domainId=invalid-uuid');
  ...
  it('should return 400 for invalid taskId UUID', async () => {
    const response = await request(app).get('/api/flashcards?taskId=not-a-uuid');
...
describe('GET /api/flashcards/review', () => {
  it('should get flashcards due for review with default limit', async () => {
    const mockFlashcards = [mockFlashcard];
    (flashcardService.getDueForReview as jest.Mock).mockResolvedValueOnce(mockFlashcards);
    ...
    expect(flashcardService.getDueForReview).toHaveBeenCalledWith(mockUserId, 20);
  });
  ...
  it('should get flashcards due for review with custom limit', async () => {
    ...
    expect(flashcardService.getDueForReview).toHaveBeenCalledWith(mockUserId, 30);
  });
...
describe('GET /api/flashcards/stats', () => {
  it('should get user review statistics', async () => {
    (flashcardService.getReviewStats as jest.Mock).mockResolvedValueOnce(mockReviewStats);
...
describe('POST /api/flashcards/sessions', () => {
  it('should start a new flashcard session with default options', async () => {
    ...
    (flashcardService.startSession as jest.Mock).mockResolvedValueOnce(mockSession);
  ...
  it('should start session with specific domains', async () => {
    ...
    const response = await request(app).post('/api/flashcards/sessions').send(requestBody);
  ...
  it('should start session with custom options', async () => {
    ...
    expect(flashcardService.startSession).toHaveBeenCalledWith(mockUserId, requestBody);
  });
...
describe('GET /api/flashcards/sessions/:id', () => {
  it('should get an existing session', async () => {
    ...
    (flashcardService.getSession as jest.Mock).mockResolvedValueOnce(mockSession);

    const response = await request(app).get(`/api/flashcards/sessions/${mockSessionId}`);
    ...
    expect(flashcardService.getSession).toHaveBeenCalledWith(mockSessionId, mockUserId);
  });
  ...
  it('should return 400 for invalid session ID UUID', async () => {
    const response = await request(app).get('/api/flashcards/sessions/invalid-uuid');
  ...
  it('should handle service errors', async () => {
    (flashcardService.getSession as jest.Mock).mockRejectedValueOnce(new Error('Database error'));
...
describe('POST /api/flashcards/sessions/:id/responses/:cardId', () => {
  it('should record response with know_it rating', async () => {
    (flashcardService.recordResponse as jest.Mock).mockResolvedValueOnce(undefined);
...
describe('POST /api/flashcards/sessions/:id/complete', () => {
  it('should complete a flashcard session', async () => {
    ...
    expect(flashcardService.completeSession).toHaveBeenCalledWith(mockSessionId);
  });

  it('should return 400 for invalid session ID UUID', async () => {
    const response = await request(app).post('/api/flashcards/sessions/invalid-uuid/complete');
...
describe('POST /api/flashcards/custom', () => {
  it('should create a custom flashcard', async () => {
    ...
    const response = await request(app).post('/api/flashcards/custom').send(requestBody);
...
describe('Property-based testing for validation', () => {
  ...
  it('should accept all valid FlashcardRating values', async () => {
    ...
    const validRatings: FlashcardRating[] = ['know_it', 'learning', 'dont_know'];
  ...
  it('should handle various numeric limit values correctly', async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 100 }), async validLimit => {
        (flashcardService.getFlashcards as jest.Mock).mockResolvedValueOnce([]);
        const response = await request(app).get(`/api/flashcards?limit=${validLimit}`);
        expect(response.status).toBe(200);
  ...
  it('should handle various cardCount values within valid range', async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 100 }), async validCardCount => {
