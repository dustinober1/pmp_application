...
const DEFAULT_RULES: SanitizationRule[] = [
  ...
  { field: 'authorization', pattern: /Bearer .+/g, replacement: 'Bearer [REDACTED]' },
  { field: 'ssn', pattern: /\d{3}-\d{2}-\d{4}/g, replacement: '***-**-****' },
  { field: 'creditCard', pattern: /\d{4} \d{4} \d{4} \d{4}/g, replacement: '**** **** **** ****' },
];

/**
 * Sanitize an object by redacting sensitive fields
 */
export function sanitize(obj: any, rules: SanitizationRule[] = DEFAULT_RULES): any {
