...
export class StripeService {
  ...
  async createCheckoutSession(
    ...
    return withSpan('stripe.createCheckoutSession', async span => {
      ...
      // Get user with span
      const user = await withSpan('stripe.createCheckoutSession.getUser', async dbSpan => {
      ...
      const session = await withSpan('stripe.api.createCheckoutSession', async apiSpan => {
  ...
  private async handleCheckoutCompleted(session: Stripe.Checkout.Session) {
    await withSpan('stripe.handleCheckoutCompleted', async span => {
      ...
      if (!userId || !tierId) {
        const error = new Error(`Stripe Session missing metadata: ${session.id}`);
        recordError(error, {
      ...
      await withSpan('stripe.handleCheckoutCompleted.upsertSubscription', async dbSpan => {
        setDatabaseContext(dbSpan, {
      ...
      // Record transaction
      await withSpan('stripe.handleCheckoutCompleted.createTransaction', async dbSpan => {
        setDatabaseContext(dbSpan, {
  ...
  private async handleInvoicePaymentSucceeded(invoice: Stripe.Invoice) {
    await withSpan('stripe.handleInvoicePaymentSucceeded', async span => {
      ...
      await withSpan('stripe.handleInvoicePaymentSucceeded.updateSubscription', async dbSpan => {
        setDatabaseContext(dbSpan, {
          table: 'UserSubscription',
          operation: 'update',
        });
  ...
  private async handleSubscriptionUpdated(subscription: Stripe.Subscription) {
    await withSpan('stripe.handleSubscriptionUpdated', async span => {
      ...
      span.setAttribute('user.id', userId);
      span.setAttribute('stripe.cancel_at_period_end', subscription.cancel_at_period_end);

      if (subscription.cancel_at_period_end) {
        await withSpan('stripe.handleSubscriptionUpdated.markCancelled', async dbSpan => {
          setDatabaseContext(dbSpan, {
            table: 'UserSubscription',
            operation: 'update',
          });
        ...
        await withSpan('stripe.handleSubscriptionUpdated.updateActive', async dbSpan => {
          setDatabaseContext(dbSpan, {
            table: 'UserSubscription',
            operation: 'update',
          });
  ...
  async cancelSubscription(userId: string): Promise<void> {
    return withSpan('stripe.cancelSubscription', async span => {
      ...
      await withSpan('stripe.cancelSubscription.updateStripe', async apiSpan => {
        setExternalApiContext(apiSpan, {
          name: 'Stripe',
          method: 'POST',
          url: `https://api.stripe.com/v1/subscriptions/${subscription.stripeSubscriptionId}`,
        });

        return stripe.subscriptions.update(subscription.stripeSubscriptionId, {
  ...
  async createBillingPortalSession(userId: string): Promise<{ url: string }> {
    return withSpan('stripe.createBillingPortalSession', async span => {
      ...
      const session = await withSpan('stripe.api.createBillingPortalSession', async apiSpan => {
        setExternalApiContext(apiSpan, {
          name: 'Stripe',
          method: 'POST',
