...
export class DashboardService {
    ...
    async getRecentActivity(userId: string, limit: number = 10): Promise<ActivityResponse[]> {
        ...
        return activities.map(a => ({
            id: a.id,
            type: a.activityType,
            description: this.getActivityDescription(a.activityType, a.metadata as Record<string, unknown>),
    ...
    async getReadinessScore(userId: string): Promise<ReadinessResponse> {
        ...
        const recommendation = this.getReadinessRecommendation(overallScore, contentScore, practiceScore, retentionScore);
     ...
     */
    private async getPracticeOverview(userId: string): Promise<{ averageAccuracy: number; totalAttempts: number }> {
        const attempts = await prisma.questionAttempt.findMany({
            where: { userId },
        ...
        const averageAccuracy = totalAttempts > 0 ? Math.round((correctCount / totalAttempts) * 100) : 0;
     ...
     */
    private async getFlashcardOverview(userId: string): Promise<{ masteredPercentage: number; totalCards: number }> {
        const reviews = await prisma.flashcardReview.findMany({
            where: { userId },
        });
    ...
    private getActivityDescription(type: string, metadata: Record<string, unknown> | null): string {
        switch (type) {
            ...
            case 'practice_complete':
                ...
                return score ? `Completed practice session with ${score}% score` : 'Completed practice session';
            case 'mock_exam':
