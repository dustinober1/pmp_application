...
export class FormulaService {
    ...
    async getFormulas(category?: FormulaCategory): Promise<FormulaResponse[]> {
        ...
        return formulas.map(f => ({
            ...
            variables: f.variables.map((v): VariableResponse => ({
                symbol: v.symbol,
                name: v.name,
    ...
    async getFormulaById(formulaId: string): Promise<FormulaResponse | null> {
        ...
        return {
            ...
            variables: formula.variables.map((v): VariableResponse => ({
                symbol: v.symbol,
                name: v.name,
                description: v.description,
                unit: v.unit || undefined,
    ...
    private calculateEarnedValue(
        ...
        switch (formulaName.toUpperCase()) {
            ...
            case 'COST PERFORMANCE INDEX':
                steps.push({ stepNumber: 1, description: 'CPI = EV / AC', expression: `${EV} / ${AC}`, value: EV / AC });
                return EV / AC;

            case 'SPI':
            case 'SCHEDULE PERFORMANCE INDEX':
                steps.push({ stepNumber: 1, description: 'SPI = EV / PV', expression: `${EV} / ${PV}`, value: EV / PV });
                return EV / PV;

            case 'CV':
            case 'COST VARIANCE':
                steps.push({ stepNumber: 1, description: 'CV = EV - AC', expression: `${EV} - ${AC}`, value: EV - AC });
                return EV - AC;

            case 'SV':
            case 'SCHEDULE VARIANCE':
                steps.push({ stepNumber: 1, description: 'SV = EV - PV', expression: `${EV} - ${PV}`, value: EV - PV });
                return EV - PV;
            ...
            case 'ESTIMATE AT COMPLETION':
                if (CPI) {
                    ...
                    steps.push({ stepNumber: 1, description: 'EAC = BAC / CPI', expression: `${BAC} / ${CPI}`, value: result });
                    return result;
                }
                const result = AC + (BAC - EV);
                steps.push({ stepNumber: 1, description: 'EAC = AC + (BAC - EV)', expression: `${AC} + (${BAC} - ${EV})`, value: result });
                return result;

            case 'ETC':
            case 'ESTIMATE TO COMPLETE':
                const etc = EAC - AC;
                steps.push({ stepNumber: 1, description: 'ETC = EAC - AC', expression: `${EAC} - ${AC}`, value: etc });
                return etc;

            case 'VAC':
            case 'VARIANCE AT COMPLETION':
                const vac = BAC - EAC;
                steps.push({ stepNumber: 1, description: 'VAC = BAC - EAC', expression: `${BAC} - ${EAC}`, value: vac });
                return vac;
            ...
            case 'TO-COMPLETE PERFORMANCE INDEX':
                if (EAC > 0) {
                    ...
                    steps.push({ stepNumber: 1, description: 'TCPI = (BAC - EV) / (EAC - AC)', expression: `(${BAC} - ${EV}) / (${EAC} - ${AC})`, value: tcpi });
                    return tcpi;
                }
                const tcpiBac = (BAC - EV) / (BAC - AC);
                steps.push({ stepNumber: 1, description: 'TCPI = (BAC - EV) / (BAC - AC)', expression: `(${BAC} - ${EV}) / (${BAC} - ${AC})`, value: tcpiBac });
                return tcpiBac;
    ...
    private calculateScheduling(
        ...
        switch (formulaName.toUpperCase()) {
            ...
            case 'PERT ESTIMATE': {
                ...
                steps.push({ stepNumber: 1, description: 'PERT = (O + 4M + P) / 6', expression: `(${O} + 4×${M} + ${P}) / 6`, value: result });
                return result;
            }
            ...
            case 'STANDARD DEVIATION': {
                ...
                steps.push({ stepNumber: 1, description: 'σ = (P - O) / 6', expression: `(${pessimistic} - ${optimistic}) / 6`, value: result });
                return result;
            }
            ...
            case 'VARIANCE': {
                ...
                const result = sd * sd;
                steps.push({ stepNumber: 1, description: 'Variance = σ²', expression: `${sd}²`, value: result });
                return result;
            }
    ...
    private calculateCost(
        ...
        switch (formulaName.toUpperCase()) {
            ...
            case 'RETURN ON INVESTMENT': {
                ...
                steps.push({ stepNumber: 1, description: 'ROI = (Benefit - Cost) / Cost × 100', expression: `(${benefit} - ${cost}) / ${cost} × 100`, value: result });
                return result;
            }
            ...
            case 'NET PRESENT VALUE': {
                ...
                steps.push({ stepNumber: 1, description: 'PV = CF / (1 + r)^n', expression: `${cashFlow} / (1 + ${rate})^${periods}`, value: pv });
                return pv;
            }
            ...
            case 'PAYBACK PERIOD': {
                ...
                steps.push({ stepNumber: 1, description: 'Payback = Investment / Annual Cash Flow', expression: `${investment} / ${annualCashFlow}`, value: result });
                return result;
            }
    ...
    private calculateCommunication(
        ...
        switch (formulaName.toUpperCase()) {
            case 'COMMUNICATION CHANNELS': {
                ...
                steps.push({ stepNumber: 1, description: 'Channels = n(n-1) / 2', expression: `${n} × (${n} - 1) / 2`, value: result });
                return result;
            }
