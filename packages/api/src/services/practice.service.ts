...
export class PracticeService {
    ...
    async completeSession(sessionId: string, userId: string): Promise<PracticeSessionResult> {
        ...
        const domainBreakdown: DomainScore[] = Array.from(domainMap.entries()).map(([domainId, data]) => ({
            domainId,
     ...
     */
    async startMockExam(userId: string): Promise<{ sessionId: string; questions: PracticeQuestion[] }> {
    ...
    async getFlaggedQuestions(userId: string): Promise<PracticeQuestion[]> {
        ...
        return flagged.map(f => ({
            ...
            options: f.question.options.map((o): QuestionOption => ({
                id: o.id,
                questionId: o.questionId,
                text: o.text,
                isCorrect: false,
