...
describe('ContentService', () => {
  ...
  describe('getDomains', () => {
    ...
    it('should handle database errors', async () => {
      ...
      await expect(contentService.getDomains()).rejects.toThrow('Database connection failed');
    });
  });
  ...
  describe('getUserProgress', () => {
    it('should calculate overall progress correctly', async () => {
      ...
      const mockCompletedProgress = [
        {
          ...
          section: {
            ...
            studyGuide: {
              taskId: 'task-1',
              task: { domainId: 'domain-1', domain: { id: 'domain-1', name: 'People' } },
            },
          },
        },
        {
      ...
      (prisma.studyProgress.findMany as jest.Mock).mockResolvedValue(mockCompletedProgress);
          ...
          tasks: [
            {
              id: 'task-1',
              studyGuide: {
                sections: Array.from({ length: 10 }, (_, i) => ({ id: `section-${i}` })),
              },
            },
          ],
        },
  ...
  describe('searchContent', () => {
    it('should search across study guides, flashcards, and questions', async () => {
      ...
      (prisma.studySection.findMany as jest.Mock).mockResolvedValue(mockSections);
      (prisma.flashcard.findMany as jest.Mock).mockResolvedValue(mockFlashcards);
      (prisma.practiceQuestion.findMany as jest.Mock).mockResolvedValue(mockQuestions);
