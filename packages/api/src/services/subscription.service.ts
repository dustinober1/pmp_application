import { SubscriptionTier, UserSubscription, TierFeatures, TierName, SUBSCRIPTION_ERRORS } from '@pmp/shared';
...
export class SubscriptionService {
    ...
    async createSubscription(
        ...
        if (!tier) {
            throw AppError.badRequest(SUBSCRIPTION_ERRORS.SUB_002.message, SUBSCRIPTION_ERRORS.SUB_002.code);
        }

        // Calculate end date (1 month or 1 year based on billing period)
        const durationMs = tier.billingPeriod === 'annual'
            ? 365 * 24 * 60 * 60 * 1000
            : 30 * 24 * 60 * 60 * 1000;
