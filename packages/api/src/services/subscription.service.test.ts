...
describe("SubscriptionService", () => {
  ...
  describe("getTiers", () => {
    ...
    it("should handle database errors", async () => {
      ...
      await expect(subscriptionService.getTiers()).rejects.toThrow(
        "Database error",
      );
    });
  });
  ...
  describe("getTierByName", () => {
    ...
    it("should return null when tier not found", async () => {
      ...
      const result = await subscriptionService.getTierByName(
        "non-existent" as TierName,
      );
  ...
  describe("getUserSubscription", () => {
    ...
    it("should return user subscription with tier details", async () => {
      ...
      expect(result?.stripeSubscriptionId).toBe("sub_stripe123");
    });
  ...
  describe("createSubscription", () => {
    ...
    it("should calculate correct end date for monthly billing", async () => {
      ...
      (prisma.userSubscription.upsert as jest.Mock).mockImplementation(
        ({ create }) => {
  ...
  describe("setGracePeriod", () => {
    ...
    it("should set grace period with 7-day extension", async () => {
      ...
      (prisma.userSubscription.update as jest.Mock).mockImplementation(
        ({ data }) => {
          ...
          // Allow for small timing differences
          expect(
            Math.abs(gracePeriodEnd.getTime() - expectedEnd.getTime()),
          ).toBeLessThan(1000);
  ...
  describe("expireSubscription", () => {
    ...
    it("should downgrade to free tier when expired", async () => {
      (prisma.subscriptionTier.findFirst as jest.Mock).mockResolvedValue(
        mockFreeTier,
      );
      (prisma.userSubscription.update as jest.Mock).mockResolvedValue({});
    ...
    it("should set long expiry for free tier (10 years)", async () => {
      ...
      (prisma.userSubscription.update as jest.Mock).mockImplementation(
        ({ data }) => {
          ...
          // Allow for small timing differences
          expect(
            Math.abs(endDate.getTime() - expectedEnd.getTime()),
          ).toBeLessThan(1000);
  ...
  describe("Property-Based Tests", () => {
    ...
    it("should validate tier names", () => {
      fc.assert(
        fc.property(
          fc.constantFrom("free", "pro", "pro", "corporate"),
          (tierName) => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should create subscription without Stripe IDs for free tier", async () => {
      const userId = "user-123";
      const tierId = mockFreeTier.id;
