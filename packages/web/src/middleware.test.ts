...
describe('middleware', () => {
  ...
  describe('protected paths with auth', () => {
    it('allows access to dashboard when authenticated', () => {
      const request = createMockRequest('/dashboard', { pmp_access_token: 'valid-token' });
      middleware(request);
    ...
    it('allows access to study when authenticated', () => {
      const request = createMockRequest('/study', { pmp_access_token: 'valid-token' });
      middleware(request);
    ...
    it('allows access to flashcards when authenticated', () => {
      const request = createMockRequest('/flashcards', { pmp_access_token: 'valid-token' });
      middleware(request);
...
describe('middleware config', () => {
  it('has correct matcher pattern', () => {
    expect(config.matcher).toEqual(['/((?!_next/static|_next/image|robots.txt|sitemap.xml).*)']);
  });
