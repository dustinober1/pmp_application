...
function VerifyEmailContent() {
  ...
  const [status, setStatus] = useState<'awaiting' | 'verifying' | 'success' | 'error'>(
  ...
  const handleResend = async () => {
    ...
    setDevLink(null);
    try {
      const response = await apiRequest<{ token?: string }>('/auth/resend-verification', {
        method: 'POST',
      });
      ...
      const msg = error?.message || 'Failed to resend verification email. Please try again.';
      toast.error(msg);
      setMessage(msg);
      setStatus('error');
    } finally {
      ...
      {status === 'awaiting' && (
        <>
          ...
          <p className="text-gray-400 mb-6">
            Check your inbox for the verification link, then open it to complete email verification.
          </p>
          <div className="space-y-3">
            ...
            {devLink && (
              <div className="text-left text-sm bg-gray-900/40 border border-gray-700 rounded-lg p-3">
                <p className="text-gray-300 mb-1 font-medium">Dev verification link</p>
                <a href={devLink} className="text-primary-300 underline break-all">
                  {devLink}
                </a>
              </div>
            )}
