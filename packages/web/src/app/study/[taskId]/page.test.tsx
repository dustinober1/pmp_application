...
describe("StudyGuidePage", () => {
  ...
  const mockStudyGuide = {
    ...
    sections: [
      ...
      },
      {
        id: "section-2",
        title: "Key Concepts",
        content: "Important concepts to know.",
      },
    ],
  };
  ...
  it("renders task name and description after loading", async () => {
    ...
    expect(
      screen.getByText("Learn the basics of project management"),
    ).toBeInTheDocument();
  });
  ...
  it("renders section content", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("This is the intro content."),
      ).toBeInTheDocument();
    });
  ...
  it("shows no content sections message when sections empty", async () => {
    mockApiRequest.mockImplementation((url: string) => {
      ...
      if (url.includes("/study-guide")) {
        return Promise.resolve({
          data: { studyGuide: { id: "guide-1", sections: [] } },
        });
      }
      return Promise.resolve({ data: null });
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No content sections available."),
      ).toBeInTheDocument();
    });
  });
});
