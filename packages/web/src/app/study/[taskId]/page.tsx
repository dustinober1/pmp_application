  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setLoading(true);
        // Fetch task details
        const tasksResponse = await apiRequest<{ tasks: Task[] }>('/domains/tasks');
        const foundTask = tasksResponse.data?.tasks.find(t => t.id === taskId);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-2 text-sm text-gray-400 mb-2">
          <button onClick={() => router.push('/study')} className="hover:text-white transition">
            Study Guide
          </button>
          <span>/</span>
          ...
          <div className="sticky top-24 bg-gray-900 border border-gray-800 rounded-xl p-4">
            ...
            <nav className="space-y-1">
              {!guide || guide.sections.length === 0 ? (
                <p className="text-sm text-gray-500 italic">No content sections available.</p>
              ) : (
          ...
          {!guide ? (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center">
              ...
              <p className="text-gray-400">
                Detailed study guide content for this task is currently being developed. Please
                check back later or start with flashcards.
              </p>
