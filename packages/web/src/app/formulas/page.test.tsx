...
describe("FormulasPage", () => {
  ...
  it("calculates result when inputs provided", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Enter EV"), {
      target: { value: "120" },
    });
    fireEvent.change(screen.getByPlaceholderText("Enter AC"), {
      target: { value: "100" },
    });
  ...
  it("shows upgrade message for free tier users", async () => {
    ...
    expect(
      screen.getByText(
        /Interactive calculator is available for High-End and Corporate tiers/i,
      ),
    ).toBeInTheDocument();
    expect(
