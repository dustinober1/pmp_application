...
describe("TeamDashboardPage", () => {
    ...
    alerts: [
      {
        id: "alert-1",
        type: "inactive",
        message: "John Doe has been inactive for 7 days",
      },
      {
        id: "alert-2",
        type: "struggling",
  ...
  it("renders alerts section", async () => {
    ...
    expect(
      screen.getByText("John Doe has been inactive for 7 days"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Jane Smith is struggling with Process domain"),
    ).toBeInTheDocument();
  });
  ...
  it("dismissing alert removes it from list", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("John Doe has been inactive for 7 days"),
  ...
  it("shows success message after successful invitation", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Invitation sent to newuser@company.com"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows license usage information", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("You have used 5 of 10 licenses."),
      ).toBeInTheDocument();
    });
  });
      ...
      (url: string, options?: { method?: string }) => {
        ...
        if (url.includes("/dashboard")) {
          return Promise.resolve({
            data: { dashboard: dashboardWithNeverActive },
          });
        }
        return Promise.resolve({ data: null });
      },
