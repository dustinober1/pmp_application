...
describe("PracticeSessionPage", () => {
  ...
  beforeEach(() => {
    ...
    mockApiRequest.mockImplementation((url: string) => {
      if (
        url.includes("/practice/sessions/session-123") &&
        ...
        return Promise.resolve({
          data: {
            result: {
              isCorrect: true,
              explanation:
                "The critical path is the longest sequence of activities.",
  ...
  it("renders question text after loading", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("What is the critical path?"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("renders question options", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("The longest path through a project network"),
      ).toBeInTheDocument();
    });
    expect(
      screen.getByText("The shortest path through a project network"),
    ).toBeInTheDocument();
  });
  ...
  it("shows session complete screen when all questions answered", async () => {
    const completedSession = {
      ...mockSession,
      questions: mockSession.questions.map((q) => ({
        ...q,
        userAnswerId: "opt1",
      })),
  ...
  it("options are disabled after submitting answer", async () => {
    ...
    const option = screen.getByText(
      "The shortest path through a project network",
    );
