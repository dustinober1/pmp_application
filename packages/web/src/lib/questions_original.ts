...
function randomizeQuestionAnswers(question: Question): Question {
  ...
  const shuffledAnswers: QuestionAnswer[] = shuffledIndices.map((index) => {
    if (index < 0 || index >= question.answers.length) {
      throw new Error(`Invalid index ${index} for question answers`);
    }
...
function ensureEvenAnswerDistribution(questions: Question[]): Question[] {
  ...
  // Shuffle answers according to the shuffled indices
  const shuffledAnswers: QuestionAnswer[] = shuffledIndices.map(
...
function ensureEvenAnswerDistribution(questions: Question[]): Question[] {
  ...
  for (const question of questions) {
    ...
    if (randomized.correctAnswerIndex !== targetIndex) {
      const temp = randomized.answers[targetIndex];
      randomized.answers[targetIndex] =
        randomized.answers[randomized.correctAnswerIndex];
      randomized.answers[randomized.correctAnswerIndex] = temp;
      randomized.correctAnswerIndex = targetIndex;
    }
