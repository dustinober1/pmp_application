  ...
  return (
    <div className={`mt-2 ${className}`}>
      ...
      <ul className="text-xs text-gray-400 space-y-1">
        <li
          className={`flex items-center gap-2 ${hasMinLength ? "text-green-400" : ""}`}
        >
          <span>{hasMinLength ? "✓" : "○"}</span>
          <span>At least 8 characters</span>
        </li>
        <li
          className={`flex items-center gap-2 ${hasLowercase ? "text-green-400" : ""}`}
        >
          <span>{hasLowercase ? "✓" : "○"}</span>
          <span>Lowercase letter</span>
        </li>
        <li
          className={`flex items-center gap-2 ${hasUppercase ? "text-green-400" : ""}`}
        >
          <span>{hasUppercase ? "✓" : "○"}</span>
          <span>Uppercase letter</span>
        </li>
        <li
          className={`flex items-center gap-2 ${hasNumber ? "text-green-400" : ""}`}
        >
          <span>{hasNumber ? "✓" : "○"}</span>
          <span>Number</span>
        </li>
