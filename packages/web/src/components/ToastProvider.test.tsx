...
describe("ToastProvider", () => {
  ...
  it("responds to sync-failed custom event", () => {
    ...
    act(() => {
      window.dispatchEvent(
        new CustomEvent("pmp-sync-failed", { detail: { count: 3 } }),
      );
    });

    expect(screen.getByRole("alert")).toBeInTheDocument();
    expect(
      screen.getByText(/Some offline actions failed to sync \(3\)/),
    ).toBeInTheDocument();
  });
  ...
  it("handles sync-failed event without count", () => {
    ...
    expect(screen.getByRole("alert")).toBeInTheDocument();
    expect(
      screen.getByText(/Some offline actions failed to sync \(1\)/),
    ).toBeInTheDocument();
  });
