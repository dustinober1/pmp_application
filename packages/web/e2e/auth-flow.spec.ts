...
test.describe('Authentication Flow', () => {
  ...
  test.describe('User Login', () => {
    test('should login with valid credentials', async ({ page }) => {
      await authPage.goto();
      await authPage.login(testUsers.standard.email, testUsers.standard.password);
  ...
  test.describe('Logout', () => {
    ...
    test('should clear session data on logout', async ({ _authenticatedPage }) => {
      ...
      const token = await authPage.page.evaluate(() => localStorage.getItem('token'));
      expect(token).toBeNull();

      const refreshToken = await authPage.page.evaluate(() => localStorage.getItem('refreshToken'));
