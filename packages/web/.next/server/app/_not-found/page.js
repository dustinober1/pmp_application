    ...
    (e.modules = {
      ...
      2278: (e, t, r) => {
        ...
        let a = (0, n.createContext)(void 0),
          s = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api';
        function i({ children: e }) {
          let [t, r] = (0, n.useState)({
            ...
            i = async e => {
              try {
                let t = await fetch(`${s}/auth/me`, { headers: { Authorization: `Bearer ${e}` } });
                if (t.ok) {
                  let o = await t.json();
                  r({ user: o.data.user, token: e, isLoading: !1, isAuthenticated: !0 });
                } else await c();
              } catch (e) {
            ...
            l = async (e, t) => {
              let o = await fetch(`${s}/auth/login`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ email: e, password: t }),
                }),
                n = await o.json();
              if (!o.ok) throw Error(n.error?.message || 'Login failed');
              let { accessToken: a, refreshToken: i, user: l } = n.data;
              (localStorage.setItem('accessToken', a),
                localStorage.setItem('refreshToken', i),
                r({ user: l, token: a, isLoading: !1, isAuthenticated: !0 }));
            },
            d = async (e, t, o) => {
              let n = await fetch(`${s}/auth/register`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ email: e, password: t, name: o }),
                }),
                a = await n.json();
              if (!n.ok) throw Error(a.error?.message || 'Registration failed');
              let { accessToken: i, refreshToken: l, user: d } = a.data;
              (localStorage.setItem('accessToken', i),
                localStorage.setItem('refreshToken', l),
                r({ user: d, token: i, isLoading: !1, isAuthenticated: !0 }));
            },
            u = () => {
            ...
            c = async () => {
              ...
              try {
                let t = await fetch(`${s}/auth/refresh`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
          ...
          return o.jsx(a.Provider, {
            value: { ...t, login: l, register: d, logout: u, refreshToken: c },
            children: e,
          });
        }
        function l() {
