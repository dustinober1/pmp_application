...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '../packages/web-svelte/static/data/flashcards.json')));
...
for (const [domain, info] of Object.entries(domains)) {
  ...
  for (const t of info.tasks) {
    const taskName = t.task.length > 50 ? t.task.substring(0, 47) + '...' : t.task;
    console.log(`   └─ ${t.ecoRef}: ${taskName} (${t.count} cards)`);
  }
}

console.log('\n\n=== FILE SIZE ESTIMATE ===');
const totalCards = Object.values(domains).reduce((sum, d) => sum + d.totalCards, 0);
