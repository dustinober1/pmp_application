...
const inputPath = path.join(
  __dirname,
  "../packages/web-svelte/static/data/flashcards.json",
);
const outputDir = path.join(
  __dirname,
  "../packages/web-svelte/static/data/flashcards",
);
...
for (const group of data) {
  ...
  const domainId =
    DOMAIN_MAP[domainName] || domainName.toLowerCase().replace(/\s+/g, "-");
...
for (const [domainId, domainData] of Object.entries(domainGroups)) {
  ...
  console.log(
    ` ${filename}: ${domainData.totalCards} cards, ${domainData.tasks.length} tasks (${fileSizeKB} KB)`,
  );
...
// Summary
const totalCards = Object.values(domainGroups).reduce(
